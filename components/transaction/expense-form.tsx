        ...
        <Select
          ...
          <SelectTrigger className={errors.pocket ? 'border-danger' : ''}>
            {(() => {
              ...
              if (pocket === TABUNGAN_UTAMA) {
                return (
                  <>
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary-bg text-primary">
                      <PiggyBank className="h-4 w-4" />
                    </div>
                    <span className="flex-1 text-sm font-medium text-text-primary text-left">{TABUNGAN_UTAMA}</span>
                    <span className="text-xs font-mono text-text-muted">{formatRp(data?.tabunganUtama ?? 0)}</span>
                  </>
                )
              }
              return sel ? (
          ...
          <SelectContent>
            {data?.tabunganUtama !== undefined && (
              <SelectItem value={TABUNGAN_UTAMA} className="py-3">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary-bg text-primary">
                  <PiggyBank className="h-4 w-4" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-text-primary truncate">Tabungan Utama</p>
                  <p className="text-caption text-text-muted font-mono">Sisa {formatRp(data.tabunganUtama)}</p>
                </div>
              </SelectItem>
            )}
            {data?.pockets.map((p) => {
